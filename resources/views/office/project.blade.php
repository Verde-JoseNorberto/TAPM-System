  ...
  <div class="row row-cols-1 row-cols-md-2 g-4 my-2">
    @foreach($group_projects->tasks as $task)
      @foreach($task->projects as $project)
        @include('office.post', ['tasks' => $tasks, 'projects' => $projects, 'group_project_id' => $group_projects->id])
      @endforeach
      
      @foreach($task->subtasks as $subtask)
        @foreach($subtask->projects as $project)
          @include('office.post', ['subtasks' => $subtasks, 'projects' => $projects, 'group_project_id' => $group_projects->id])
        @endforeach
      @endforeach
    @endforeach
  </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="POST" action="{{ route('office/project') }}" enctype="multipart/form-data">
                    @csrf
